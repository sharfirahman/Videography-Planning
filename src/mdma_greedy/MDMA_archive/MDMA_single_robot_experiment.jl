#This file is added by Sharfi just to do the experiment with single robot and multi subject
...
#Adding Sharfi's MPCPlanner to export

export AssignmentPlanner, GreedyPlanner, FormationPlanner, MyopicPlanner, MultipleRoundsGreedyPlanner,MPCPlanner
...
actor
    ...
    #single_problem = MDMA.SingleRobotMultiTargetViewCoverageProblem(robot_states)
    multi_problem
...
function run_experiment(
    ...
    problem = init_discrete_problem(experiment_name, path_to_experiments)
    # Solving output
    println("Solving Solution for $(experiment_name) using MPCPlanner")
    #Need to add target trajectory for actors position

    #target = problem.target_trajectories[1,actor] #Need interfacing with the MPC code

    actor_trajectories = problem.target_trajectories

    x = Float64(initial_discrete.state.x)
    y = Float64(initial_discrete.state.y)
    z = drone_height
    theta = dirAngle(initial_discrete.state.heading)
    initial_continuous = [x, y, z, 0.0, 0.0, 0.0, theta, 0.0]

    # Run MPC with the actor trajectories
    trajectory, coverage_history = MPC.run_mpc_simulation(
        initial_continuous,
        planner.mpc_params,
        actor_trajectories,  # ← Pass to MPC
        sensor,
        horizon,
        1
    )
    ...
    # Convert and save
    mdp_trajectory = convert_mpc_to_mdma_trajectory(trajectory, horizon)
    total_reward = sum(coverage_history)
    
    solution = Solution(total_reward, [(1, mdp_trajectory)])
