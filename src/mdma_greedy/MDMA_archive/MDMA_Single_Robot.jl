...
function compute_single_agent_view_reward(
    ...
    for (target_id, t) in enumerate(targets)
        ...
        if detectTarget(mdp_state.state, t, model.sensor)
             println("Target detected ", t.x, " ", t.y, " ", mdp_state.state.x, " ", mdp_state.state.y)
             println()
            ...
            for (f_id, face) in enumerate(t.faces)
                ...
                # Compute marginal view reward for this face - the more a face has been viewed, the less valuable
                # additional views become
