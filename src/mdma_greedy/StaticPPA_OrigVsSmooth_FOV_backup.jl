...
function run_static_comparison(; global_max::Union{Nothing,Float64}=nothing)
    ...
    ppa_original = map(product(ys, xs)) do (y, x)
        ...
        actora = ActorState(x, y, actor_z, actor_heading, mesh, 1)
...
function run_static_comparison_out_of_fov_away(; global_max::Union{Nothing,Float64}=nothing)
    ...
    ppa_original = map(product(ys, xs)) do (y, x)
        ...
        actora = ActorState(x, y, actor_z, heading_here, mesh, 1)
        po_static, ps_static = eval_ppa_static(actora, drone_vec; focal_length=1.2, tilt=-0.35, debug=true)
        return po_static
    end
...
function run_drone_spatial_gradient_heatmap(; global_max::Union{Nothing,Float64}=nothing)
    ...
    actor = ActorState(actor_x, actor_y, actor_z, actor_heading, mesh, 1)
...
function run_combined_all_experiments(; global_max::Union{Nothing,Float64}=nothing)
    ...
    for (actor_x, actor_y, offset_deg, label) in scenarios
        ...
        actor = ActorState(actor_x, actor_y, 0.0, actor_heading, mesh, 1)
...
# ── Compute a shared color-scale max across all four static experiments ───────
# function compute_global_ppa_max()
#     mesh = build_actor_mesh(
#         actor_width=0.5, actor_depth=0.3, actor_height=0.8,
#         front_weight=1.0, side_weight=0.5, top_weight=0.25,
#         back_weight=0.2, bottom_weight=0.1
...
#     vals = Float64[]
#     for (actor_x, actor_y, offset_deg) in [(-4.0, -4.0, 0.0), (-4.0, 0.0, 180.0), (6.0, 4.0, 0.0), (6.0, -2.0, 180.0), (0.0, 0.0, 0.0), (-6.5, -6.5, 0.0), (-7.0, 7.5, 180.0), (-6.5, 3.0, 45.0)]
#         heading_toward = atan(drone_pos[2] - actor_y, drone_pos[1] - actor_x)
#         actor_heading  = heading_toward + deg2rad(offset_deg)
#         actor = ActorState(actor_x, actor_y, 0.0, actor_heading, mesh, 1)
#         po, ps = eval_ppa_static(actor, drone_vec; focal_length=1.2, tilt=-0.35)
...
# Computed by sweeping each experiment's actual (x,y) grid for the true max of
# po/ps: toward=0.1458, away=0.0538, out_of_fov=0.0813, out_of_fov_away=0.1468.
# Rounded up slightly for headroom between sampled grid points.
global_max_val = 0.20
